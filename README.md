...
`index.html` 의 `GIVE_URL` 에 토스 링크(`https://toss.me/아이디`)를 넣으면
헤더의 ♡ 후원 버튼과 본문 하단 안내가 켜집니다. 비워 두면 관련 UI가 통째로 숨겨지므로
링크 없이 배포해도 화면이 깨지지 않습니다.

Patreon 은 쓰지 않기로 했습니다 — 달러 결제·카드 등록·가입이 필요해
한국 사용자 이탈이 크고 수수료도 8~12% 입니다. 토스 송금 링크는 수수료가 없고
보내는 쪽은 가입할 필요가 없습니다. 대신 정기결제가 안 되므로
'한 달에 한 번, 원할 때만' 보내는 자율 후원으로 안내합니다.
