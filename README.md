...
처음 들어오면 사용 가이드 팝업이 한 번 뜹니다.
'일주일 동안 보지 않기'를 켜고 닫으면 7일간 뜨지 않습니다.

## 후원
광고를 붙이지 않는 대신 자율 후원으로 유지합니다.
`index.html` 의 `GIVE` 에서 채운 항목만 화면에 나타나고, 전부 비면 후원 UI 자체가
숨겨지므로 링크 없이 배포해도 화면이 깨지지 않습니다.

```js
const GIVE={ kakao:'', account:'', etc:[] };
```
| 항목 | 내용 |
|---|---|
| `kakao` | 카카오페이 코드송금 링크. 카카오페이 앱 → 홈 → 검색 → '코드로 송금받기' |
| `account` | 계좌 문자열. 그대로 표시되고 복사 버튼이 붙습니다 |
| `etc` | 그 밖의 후원 링크 `{label, url}` 배열 (투네이션·PayPal 등) |

**결제 수단을 한 곳에 묶지 않는 이유** — 처음에는 토스 송금 링크(`toss.me`)를
쓰려 했으나 **토스아이디 서비스가 종료**되어 쓸 수 없었습니다. 같은 일이 또 생길 수
있으므로 여러 수단을 나란히 둘 수 있게 만들었습니다.

Patreon 은 쓰지 않습니다 — 달러 결제·카드 등록·가입이 필요해 한국 사용자 이탈이
크고 수수료도 8~12% 입니다. 국내 송금 링크는 수수료가 없고 보내는 쪽은 가입할
필요가 없습니다. 대신 정기결제가 안 되므로 '원할 때만 한 번씩' 보내는 자율 후원으로
안내합니다.
...
5. 한국어 gloss 신학 감수
6. ~~AdSense 연동~~ → **광고 대신 자율 후원**으로 방향 변경. `GIVE_URL` 설정 필요
